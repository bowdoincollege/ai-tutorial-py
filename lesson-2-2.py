# Lesson 2.2: Text summarization with Ollama

# Since we are using LangChain, we can substitute the Ollama model and 
# use all the same code from lesson 1.5

# Local models, depending on the specific model, might run much slower than the
# hosted commerical models, especially if your laptop or computer doesn't have a
# powerful GPU.
...
TOKEN_MAX = 60000 # Llama 3.2 context window is 128000 tokens, set to half to leave room for additional context
...
# This technically is for OpenAI models, but will serve well enough for now
tokenSplitter = TokenTextSplitter(chunk_size=TOKEN_MAX, chunk_overlap=0)
